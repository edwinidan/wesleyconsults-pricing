...
export default function PricingCalculator() {
  ...
  const handleWhatsApp = useCallback(() => {
    ...
    const text = encodeURIComponent(
      ...
      `— WesleyConsults\n` +
      `📞 0500610780 | 📧 wesleyconsults@gmail.com`
    );
      ...
      <header className="bg-navy-500 text-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-5 sm:py-6">
          <div className="flex items-center justify-between">
            ...
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-lg bg-gold-500 flex items-center justify-center">
              <span className="text-navy-500 font-bold text-lg">W</span>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-stone-200/60">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {projectTypes.map(pt => (
                  ...
                  >
                    <span className="text-base mr-1.5">{pt.icon}</span>
                    {pt.label}
                  </button>
                ))}
              </div>
